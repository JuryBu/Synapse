...
- [x] **D1（Phase 1.5）已完成**（66f14bb + 88d177f 补漏）：Message.richTokens 持久化（messages.rich_tokens 列懒迁移）+ buildRichParts 重组算法 + 全链路（agentLoop/AgentPanel/MessageBubble/branchConversation）透传。补漏：runWorkflowFromInput 工作流路径也带 richTokens（86d177f HIGH#1）。
- [x] **LOW-2 已完成**（f857f61）：方向 A——atProviders workflow 分支 token.value/id 用 mode.id（英文 slug），displayLabel=mode.name；multiAITrigger.resolveWorkflowMode 优先级反转 id→name；WorkflowEditor.triggerExample 用 mode.name（HIGH#3 补漏 UX 引导）。
- [x] **联动①已完成**（a6c12df）：detectAtTrigger 跨相邻文本节点回看（previousTextNodeWithin + locateOffset + matchWhitespaceAt/matchCaretAt 分流）+ removeTokenSpan 末尾 normalize parent 对称。MAX_LOOKBACK_CHARS=64，遇 atomic/BR/DIV/P 块边界停。
- [x] **联动②已完成**（f857f61 + 88d177f 补漏）：atProviders.flattenTree token.value 改绝对路径 + displayLabel 相对路径。补漏：buildContextFromTokens 注入文本用 displayLabel（相对路径），避免绝对路径泄漏到 LLM 云端日志（HIGH#2）。

## M7 治理小本本（5-lens review 攒下来的 18 条 MEDIUM/LOW/NIT）

- [ ] detectAtTrigger findLastTextNode 钻进嵌套 token wrapper 风险（理论性，当前 DOM 不触发，未来加 inline 样式 wrapper 时需 acceptNode 过滤 [data-token] 祖先）。
- [ ] detectAtTrigger 累积循环 iterBudget 兜底（极端 DOM 千级空文本节点链时退出条件依赖 root 边界，加 iterBudget=128 防御）。
- [ ] MessageProps.onEdit 已 4 个可选位置参数，重构为对象参数 `{ content, attachments?, richTokens? }` 防字段扩展时参数顺序错。
- [ ] ipc/conversation.ts 抽 MESSAGE_COLUMNS 数组 + buildMessageInsertSQL，根除 INSERT 两处列序复制粘贴隐式契约；或换 better-sqlite3 named parameters。
- [ ] atProviders.withTriple 默认 displayLabel=it.label，让所有走它的类型显式带 displayLabel（不依赖『value 恰好可读』偶然）+ JSDoc 明确 displayLabel 协议。
- [ ] atProviders.flattenTree 兜底分支 `modeId || modeName`（极端旧数据无 modeId 时回落含空格 modeName 仍会被截断）→ 改为严格 require modeId，或一次性 store migration 补 id。
- [ ] **electron 真机终判**：中文 IME composition（playwright 测不了）/ @文件·@目录·@MCP 真实候选 / @对话发送注入 / @工作流分流 / @设置跳转 / /命令 / 粘贴多行+截图 / Ctrl+Enter 发送 / D1 持久化往返 / LOW-2 空格 mode 名工作流 / 联动②AI view_file 主路径 / refCount 账本守恒。
