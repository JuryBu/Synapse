...
> **step 定义（2026-06-17 用户拍板，权威）**：step = **一次 user 消息** 或 **一次 model API 往返**。**没有「工具步」之分** —— tool 结果**不单独算一个 step**。

- **step**：最小统计单位，只有两种：
  - **user step**：用户发一条消息 ＝ 1 step。用户连发两条消息 ＝ 两个 step。
  - **model step**：一次 model API 往返 ＝ 1 step。即「（上一步的）命令结果 → 发给模型 → 模型思考 + 输出（含本步发起的工具调用）」整体算**一个 step**。
  - ⚠️ **tool 结果不是独立 step**：工具/子代理的执行结果会随下一次发给模型的 API 往返被一起消费，它**附着在那次 model step 上**，不自增 step 计数。「调命令 → 出结果 → 再发给模型 → 模型思考 + 输出含工具调用」整体 ＝ **一个 step**。
- **轮次（round）= 一个 user→model 的完整循环**：
  - **user 段**：连续的 user 消息（中间没有 model 完整响应），可以是 1 条，也可以连发多条。
  - **model 段**：紧随其后的 model 响应，**含其间所有工具调用、子代理派发与回收**——这些只是把多个 model step 串起来，**不增加轮次**（且 tool 结果不另计 step，见上）。
  - **轮次起点**：对话空闲（model 已响应完）之后出现的**第一条新 user 消息**。
  - user 已发、model 还没回 ＝ 新一轮**已经开始**。
  - 即：一轮 ＝ `[本轮 user 段全部消息] + [本轮 model 段全部 model step（每个 model step 含其内联的工具/子代理调用与结果）]`。
- **压缩触发判定是 step 粒度**（可以落在某一轮中间的 step），但**保留原文与 record 批次的边界一律按轮次保底**（向轮边界取整，不在轮中间切）。
- **影响范围**：BPC 的 `snapshotStepCursor` / `targetReplaceStep`（§8）、δ 延迟、step 游标计数，全部以本定义为准 —— 即 step 推进只在「新 user 消息」与「新 model API 往返」时发生，工具结果回流不推进 step 游标。
...
- **与「已有批次永不重写」（§0、`recordStore` 契约）的协调**：新增「归档批 archived」概念——**原始批仍留库**（`record_read` 可展开、UI 照常显示，满足 §0.2「UI/本地永不删减」），**仅请求体注入视图**用折叠产物。即「永不删减原文、仅折叠请求体视图」。
  > ✅ **已拍板确认（2026-06-17）**：record 折叠 =「**不删原文、只折叠请求体视图**」确为用户本意。原始批永久留库、`record_read` 与 UI 完整可见，折叠产物仅用于发给模型的注入前缀。本条核对一致，非待定项。
...
2. **压好即可用、δ 为最晚上限的无缝替换**（2026-06-17 用户拍板修订）：BPC **压好（generating→ready）后，下一轮 run 发请求就能用** BPC 前缀；`1+δ` steps **只是「最晚替换上限」，不是「必须等到」的硬性时点**。即：
   - 正常路径：BPC ready 后，**最近一次发请求时**（下一个 step 边界）即可用上 BPC 前缀，不必死等 `targetReplaceStep`。
   - δ 的真正作用：给「替换没赶上 / BPC 生成失败 / 在途被中止」留一个**兜底窗口与自动 retry 余量** —— 在到达 `targetReplaceStep` 这个最晚上限前，若 BPC 尚未就绪或替换失败，调度器可在窗口内**自动重试生成 / 重试替换**；越过上限仍未就绪才退回硬阻塞兜底（§8.2 边界）。
   - 替换动作本身仍是**无阻塞**的（只换发送视图前缀 + record，不卡当前 step）。
...
**④ 压好即用、δ 为最晚上限的无缝替换**（2026-06-17 用户拍板修订）
BPC 进入 `ready` 后，**下一次 `run()` 发请求前**即可用上 BPC 前缀（把 `apiHistory` 前缀换成 BPC 版 `recordMd`，record 已含 BPC 新批，store 原文保留最近若干轮不动），**不强制等到 `targetReplaceStep`**。`targetReplaceStep = snapshotStepCursor + 1 + δ` 仅作为**最晚替换上限**：
- ready 早于上限 → 在最近的 step 边界就替换（越早越好，减少高位停留）。
- 到上限仍未 ready（生成慢 / 失败 / 被中止）→ 在 δ 窗口内**自动重试**（重新生成或重新替换）；这是 δ 存在的核心价值（给替换没赶上 / 失败留 retry 余量）。
- 越过上限仍无可用 BPC → 退回 §8.2 边界的硬阻塞兜底（不再等 BPC）。
替换动作「无阻塞」——只换发送视图前缀，不卡当前 step。成功后插一条「BPC 自动压缩」分隔线（复用 `batchDividerByIdx`，标记该批 `source='bpc'`）。
...
> **强约束（2026-06-17 用户拍板）**：**所有可调参数都必须在设置面板有可调区域，不允许只写死在代码里**。覆盖范围包括但不限于：
> - 压缩相关：`bpcThreshold`（BPC 阈值）、`compactThreshold`（硬压缩阈值）、`bpcDeltaSteps`（δ）、`bpcAbortCooldownMin`（中止冷却）、`circuit-broken` 熔断的循环间隔阈值（§8.2 边界 5）。
> - record 分层相关：`STABLE_HEAD_FULL`(H) / `STABLE_TAIL_FULL`(T) / `SKELETON_TITLE_THRESHOLD`(M) / `RECORD_MAX_RATIO`（§2.5.4）。
> - 任何后续新增的「可调阈值 / 间隔 / 比例」常量。
> 落地要求：在 SettingsPanel 的压缩设置区（M5-BPC-7）以滑杆 / 数字输入暴露，配 §8.2 边界 4 的风险校验；本对话级可覆盖项额外提供命令 / 面板入口。**「写死常量」只允许作为默认值，不允许作为唯一来源。**

### 8.4 拍板记录 + 仍待拍板点

**✅ 已拍板（2026-06-17）：**
- **δ 与替换时机语义**：`targetReplaceStep = snapshotStepCursor + 1 + δ`，**δ 是「最晚替换上限」而非「必须等到」** —— BPC 压好（ready）后下一轮 run 发请求即可用，δ 窗口供「替换没赶上 / 失败」自动 retry 兜底（见 §8.1 第 2 点 / §8.2 ④）。
- **step 是否含 tool step**：**不含** —— step = 一次 user 消息 或 一次 model API 往返，tool 结果不单独算 step（见 §1）。
- **BPC 替换是否必须严格等到 `targetReplaceStep`**：**不必** —— ready 即可在下一个 run 用（同上）。

**仍待用户最终拍板（不阻塞排期）：**
- **熔断「连续两次无缝循环」的精确 step 间距阈值**：替换后下一次 `evaluateWater` 立即（同 step 或仅 +1 step 内）又 ≥ bpcThreshold，连续 2 次——间距阈值需确认（该阈值也须按上文强约束进设置面板可调）。
