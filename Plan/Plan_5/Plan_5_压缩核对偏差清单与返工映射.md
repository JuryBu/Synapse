...
| UI-9 | **HTML 渲染 iframe 滚动条与应用不统一**（真机反馈图二 vs 图三）：渲染模式 iframe 是独立文档，内部用浏览器默认滚动条，与应用其它区域（code/csv 等）的自定义美化滚动条（`::-webkit-scrollbar`）格格不入。→ 在 HtmlViewer 构造 srcDoc 时往 `<head>` 注入一段与全局同款的滚动条 CSS（`::-webkit-scrollbar`/track/thumb，沿用应用滚动条配色），让渲染页滚动条与应用一致；注意只注样式不破坏页面自身样式。 | small |
| UI-10 | **打开文件互相替换、中部只能存一个**（真机反馈图四/五）：M4-3 预览 tab（单击=预览临时位、双击=固定）使单击不同文件总替换同一预览位，用户感知「只能开一个」。用户要「每次点开都是独立 tab、互不影响、顶部栏切换」。→ 调整打开语义：**单击文件即开独立固定 tab（不替换）**，多文件并存由 TabBar 切换；预览替换语义降为非默认（`previewEnabled` 默认 false，保留在 `...` 菜单「Enable Preview Editors」给想要 VS Code 式预览的用户开）。改 `editorTabs` openTab 默认 `isPreview=false` + previewEnabled 默认值 + Sidebar/QuickOpen 打开调用口径。 | medium |

> stage 总数：压缩返工 M5-1~M5-7 共 7 + M5-RL 6 + M5-BPC 9 + M5-TB 7 + M5-SA 7 = 36；2026-06-17 真机验收新增 M5-FIX 10 + M5-UI 10 = 20 → **共 56 stage**（压缩返工 36 + 真机回归/打磨 20）。
