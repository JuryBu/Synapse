...
## 一、图索引（已逐张 Read + sha256 对账 + 比对对话原文「图几」，16 张）

> ⚠️ 校正记录（子代理核对）：**文件名尾号 ≠ 主人发图顺序**——存图的子代理给 327 那组起名时没按附件顺序，导致 r000327-1~5 的尾号和主人口中「图一~图五」完全错位。下表「主人图几」列以对话附件出现顺序（=主人发送顺序）为准，已用 sha256 把磁盘文件对到对话附件，逐张交叉验证。第三批主人图序：图一~图五在轮327、图六~图十在轮329、图十一在轮331。

| 文件 | 主人图几 | 实际内容（已 Read） | 对应反馈 |
|---|---|---|---|
| `images/r000325-1.webp` | — | Synapse **左侧「对话历史」面板/列表**（搜索框 + 当前/全局/全部 tab + 对话卡片列表），非 Context window | 验收上下文（D/列表相关旁证） |
| `images/r000325-2.webp` | — | Synapse Chat：AI 用 `read_conversation` 演示查到当前对话 ID（conv-1782111172549-udk1gq）+ 右侧弹出**对话切换浮层** | 对话 ID 注入旁证（第二批 C） |
| `images/r000325-3.png` | — | **Codex 右键菜单**（置顶/重命名/归档/标记未读 ── 在资源管理器中打开/复制工作目录/复制会话ID/复制深度链接 ── 派生到本地/派生到新工作树 ── 在新窗口中打开）。⚠️ **与 `r286-3.png` 是同一张图**（sha256 相同），是第二批 D 的关键参考 | 第二批 D（右键菜单参考） |
| `images/r000326-1.png` | — | **真正的 Context window 截图**：Context window 530.5k/1M (53%) + Plan usage + 5-hour limit（顶部有验收清单文字「⑤行内diff ⑥导航按轮 ⑦三框…」） | 无（验收上下文） |
| `images/r000327-3.png` | **图一** | review changes 视图：`@@ -0,0 +1,8 @@` + 「段 -0,0 +1,8」+ 绿色新增行 + 「快照已就绪 snapshot_…」 | **#1 @@ 是什么** |
| `images/r000327-4.webp` | **图二** | 收到消息测试.md **在编辑器打开文件本体（普通 Markdown 预览，无任何红绿/√×）** + 右侧对话 + 「1 个文件待审查 +10 -0」。主人用它指「打开文件本体的入口/现状」，希望在此叠加红绿√× | **#2 现状（文件本体入口，目前无 inline diff）** |
| `images/r000327-5.webp` | **图三** | 收到消息测试.md 的 SingleDiffView / review changes 风格界面（`@@ -0,0 +1,8` / `@@ -6,3 +6,13` + 接受此块/接受此段/接受全文件） | **#2 现状（做偏了，做成了 review 界面）** |
| `images/r000327-2.webp` | **图四** | **反重力 Antigravity 截图**：test_file_0623.txt 在编辑器打开，第3行「今天是个好日子呢，主人！」红底（删除行），行尾有 ✓ ✗ 放大 三个按钮（inline 装饰） | **#2 期望效果（参照样式）** |
| `images/r000327-1.webp` | **图五** | 「测试Synapse平台」对话：task_boundary 多步 + 用户「插入测试消息」+ 两条「思考中」并存 | **#5 双流** |
| `images/r000329-2.webp` | **图六** | 「3 个文件待审查」全是同一个「收到消息测试.md」(+10-0 / +13-0 / +8-11)，没合并 | **#6/#10 冗余** |
| `images/r000329-4.webp` | **图七** | ref_new.md 预览：表格渲染了**但 `**序号**` 等加粗语法未解析（显示字面 **）、无边框线、列对不齐** | **#7 表格** |
| `images/r000329-1.webp` | **图八** | Artifact「Harness 工具关系图」：`flowchart TD U[用户]-->UI...` **渲染成纯文本源码、没成流程图** | **#7 Mermaid 图** |
| `images/r000329-3.webp` | **图九** | 「思考中」+ 1 个文件待审查(+12-10)；左侧露出 KaTeX 公式 8/8=1（KaTeX 已 OK 旁证） | #7 KaTeX OK 旁证 |
| `images/r000329-5.webp` | **图十** | 一堆「接受失败：文件已在 AI 修改后继续变化，已停止文件级审阅：收到消息测试.md」通知堆叠（7 个） | **#10 接受失败** |
| `images/r000331-1.webp` | **图十一** | 「继续文件增删改排队测试」卡片【已中止】+ 7 个文件待审查（收到消息测试.md ×7：+10/+13/+8/+37/+25…）+ 输入框 /compact | **#12 中止卡 + 冗余** |
...
- [ ] **#5【P0】插入消息(interrupt)→ 双 agent 多重请求、且中断不了**
  ...
  - 图（主人「图五」）：`r000327-1.webp`（task_boundary 多步进行中 + 用户「插入测试消息」+ 两条「思考中」并存=双流铁证）
...
- [ ] **#12【P0】中止后一直卡「中止中」UI、点了没用**
  ...
  - 图（主人「图十一」）：`r000331-1.webp`（卡片「已中止」但下方还卡着一堆待审查 + 输入框试 /compact）
...
- [ ] **#6/#10【HIGH】同一文件多次修改不合并 +/−，堆成多条冗余 → 接受失败卡死**
  ...
  - 图：**主人「图六」** `r000329-2.webp`（3 条全「收到消息测试.md」）、**主人「图十」** `r000329-5.webp`（一堆「接受失败：文件已在 AI 修改后继续变化，已停止文件级审阅」）、**主人「图十一」** `r000331-1.webp`（7 条同名待审查）
...
- [ ] **#2【需重做】行内 diff 理解偏差：要文件本体红绿+√×，不是 review changes 界面**
  ...
  - 图（主人图序已 sha256 核对）：**期望样式=主人「图四」 `r000327-2.webp`**（反重力 Antigravity：打开文件本体，删除行红底、行内 ✓✗ 放大按钮）；**「图二」 `r000327-4.webp`**（主人指「打开文件本体的入口/现状」——目前是普通预览、无红绿√×，要在这上面叠加 inline diff）；**做偏的现状=主人「图三」 `r000327-5.webp`**（SingleDiffView 做成了 review changes 界面风格，方向错）。⚠️ 注意：主人原话「图二这样的红绿√×」是说「要在图二这个文件本体场景上显示成图四那样」，并非 r000327-4 本身已有红绿√×。
...
- [ ] **#7【MEDIUM】Markdown 表格 + Mermaid 图渲染不行（KaTeX 已 OK）**
  ...
  - 图：**主人「图七」=表格** `r000329-4.webp`（`**序号**` 加粗语法未解析显示字面 `**`、无边框线、列对不齐）；**主人「图八」=Mermaid** `r000329-1.webp`（flowchart TD 渲染成纯文本源码、没成流程图）
