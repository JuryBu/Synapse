...
### ⏳ 收尾待办（非新功能）
1. 🔴 **worktree 中文路径 bug**(M2-5 真机暴露)：中文路径(VC工具包)下 electron 主进程 spawn git "cannot change to 乱码" fatal→worktree 在主人环境实际用不了。node 系统层三方式全 OK 复现不了→需中文环境 electron 主进程实测修(候选:spawn cwd设ASCII tmpdir / args -C走8.3短路径)。
