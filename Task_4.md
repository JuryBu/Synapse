...
- 2026-06-16 ✅ **M2-5 worktree 按需——3 审查代理 high/medium 修复（编译双过，待真机）**：
  ...
  - ✅ 真机验证(子代理 a6a144e8)：逻辑层全过(无活动 worktree 短路零回归 / byContext map / enter 重定向规则 / exit 回主工作区 / 跨对话不串 / 清理无残留)。
  - 🔴 真机暴露【M2-4 中文路径 worktree git IPC bug（待专项修，记小本本）】：中文路径(VC工具包)下 electron 主进程 spawn git 报 "cannot change to 乱码" 全 fatal；**node 实测系统三方式(args -C / spawn cwd / env LANG)全 exit0 OK → 是 electron-specific(系统复现不了)，根因要 electron 主进程深度实测定**(疑 spawn cwd=process.cwd 中文 或 args -C 中文被 git MSYS 按 GBK 解析)。候选修法:spawn cwd 设 ASCII tmpdir / args -C 走 8.3 短路径。worktree 按需+默认可关→不阻塞 M3，留专项(M3 用到并行隔离前修)。
- 2026-06-16 ✅ **M3-1a 真子代理引擎落地 + 真机验证全过**（commit；ultracode 实现 socket 断但改完 10 文件+608/-117 + 对抗审查 3lens 无 high + 5medium 修 + 编译过）：
  - spawnSubagent 单次问答 → 真工具循环(client+tool_calls+toolRegistry.execute 多轮 SUBAGENT_MAX_ROUNDS15，对齐主 agentLoop 消息格式，不碰 run 不污染主对话) + maxDepth 派发深度(默认1不许派,>1传depth-1逐层递减必终止,buildSubagentTools 按depth含/剔spawn_subagent) + contextId=subagentId隔离(复用M2-5 byContext) + 子对话存conversations(parent+is_subagent懒迁移,listConversationSummaries过滤不进历史)+回插。
  - 5 medium 修：墙钟超时双看门狗(静默90s+硬600s)/lastError假成功收紧/fileChangeTracker改byContext分桶(并发隔离)。
  - ✅ 真机(子代理 a6d09127)全过：子对话5条含 **tool 消息=真工具循环铁证**(非单次问答)/maxDepth1无spawn_subagent/子对话落库parent+is_subagent且不进普通历史/不污染主对话(pollutionDelta0)/UI端到端主AI调spawn_subagent→tool回插→主AI总结/本地API扛3次工具循环/精确kill没误伤。
  - 📌 M3-1a low小本本(非缺陷)：工作区/workspace虚拟根 list_dir 只一层；SettingsPanel retrying 未渲黄色(M3-3卡片做)；toolPermissions字段未实装(子代理工具不受限有意)；spawn_subagent泛型systemPrompt没复用模式预设；子代理审批不标来源。
- 2026-06-16 ✅ **M3-2 固定工作流（运行器+@触发）落地 + 端到端真机验证全过**：
  - M3-2a(wbohlisdl)：MultiAIMode 升级节点编排(WorkflowNode agent/parallel/condition 串/并/判断,向后兼容旧 subagents)+runWorkflow 运行器(复用 M3-1a,节点结果传递,判断节点 onFalse=abort 中止反馈)+内建找茬模式示例。对抗审查 9high-med 修(2high:spawnSubagent 前导段入try+allSettled / evaluateCondition 超时看门狗;7medium:注入隔离/全失败区分abort/工作流护栏 MAX_NODES50+墙钟30min/可中断)。commit。
  - M3-2b(w58r795mk)：multiAITrigger.ts @MultiAI:名 触发→runWorkflow 接入对话+结果汇总。对抗审查 5high-med 修(1high:Stop 按钮失效→handleStop 并 abortAll;medium:附件泄漏 release/跨对话串台 scopedId 守卫/全角冒号)。commit。
  - ✅ M3-2 端到端真机验证全过(子代理 a09185c1，重派一次——前一个 a94813b3 infra 异常返回 system prompt 未执行)：@触发识别(@MultiAI:找茬模式走工作流)/runWorkflow 引擎节点推进(推进者 agent 串行→3找茬 parallel 并行,runningSubagents 实时流转)/Stop 中止(abortAll 生效,在途子代理变error,工作流 aborted 不继续推进,汇总「无法推进:用户终止」)/匹配失败友好提示+可用模式列表/普通对话零回归。**固定工作流能从对话@跑通坐实=Multi-AI 核心可用形态成立**。
  - 📌 M3-2 small本本：匹配失败走对话内 assistant 消息非 toast(实际更好可回看);工作流暂不吃图(GC止血,M3-3增强);ConversationList 切换不防 isStreaming(AgentPanel 侧守卫已止后果)。
- 2026-06-16 ✅ **M3-3 工作流可视化落地 + 真机验证全过**：
  - M3-3a(wzwqpp4kc,impl ECONNRESET 但改完9文件+审查2medium修)：multiAI workflowRuns[]运行实例+runWorkflow全程dispatch+对话流 WorkflowCard 四色实时(complete灰/running蓝/retrying黄/error+aborted红)+MessageBubble接入。修:workflowRuns FIFO20+persist剔运行态/abortAll同步回填子代理时序裂缝。commit。
  - M3-3b(w02cnnk65,审查0high/med)：editorTabs加workflow tab type+EditorArea case+新建WorkflowView.tsx(左列子代理四色+点进右区子代理完整对话流复用MessageBubble只读)+WorkflowCard onClick→openWorkflowTab。核心:子代理conv id≠subagentId→WorkflowRunSubagent.conversationId桥接回填→loadConversationSnapshot读子对话。commit。
  - ✅ M3-3 真机验证全过(子代理 a4131f1b,截图逐帧)：卡片四色实时(进度1/4→5/5+子代理4→5动态+耗时增长+蓝→灰流转+toast)/点卡片开中间WorkflowView tab/点进子代理看完整对话流(推进者/逻辑找茬真实非空各角色不同)/四色精确(complete rgb100,116,139灰/running rgb59,130,246蓝/error红,卡片视图一致)/找茬模式完整跑通(推进→3找茬并行→判断→修复5/5,真跑gpt-5.5)。
- 🎉 **M3 主体全部完成 + 全真机验证**：引擎 M3-1a(真子代理工具循环/maxDepth/隔离/回插) + 运行器+@触发 M3-2(串/并/判断/Stop中止) + 卡片+中间视图 M3-3(四色实时/点进子代理对话)。
- 2026-06-16 ✅ **M3-2c 工作流编辑UI落地 + 真机验证全过**(commit;实现+对抗审查6high-med修)：WorkflowEditor.tsx 新建/编辑/删除工作流模板(内建只读可复制/自定义编辑删,节点agent/parallel/condition增删配置,@触发提示,草稿模式不留空壳,id防碰撞,模板名唯一性校验)。**high toolPermissions契约断裂→方案A实装**(toolRegistry 15工具标permissionCategory+buildSubagentTools按权限过滤,根治M3-1a遗留)。✅真机(a946724a)7项全过:模板列表/新建编辑表单/持久化/**@触发编辑产出模板能真跑(新建测试流程→@MultiAI:测试流程→卡片1/1完成+真实结构化产出,WorkflowEditor→persist→runWorkflow全链路契约一致)**/内建复制带全4节点/删除自定义+内建不可删/新建取消不留空壳。

## 🎉🎉 Plan_4 全成型 + 全真机验证（2026-06-16 收官）
- **M1 上下文 harness**：conversation 原文 + record 多批次压缩 + memory 主动记忆（全真机）。
- **M2 对话体系**：record R1多批次/R3渐进读/R4 90%触发/R5崩溃恢复/R6附件分离 + M2-1回溯clamp/M2-3对话分支/M2-4 worktree管理/M2-5 worktree按需/M2-6对话级元数据/M2-S稳定性（全真机）。
- **M3 Multi-AI**：M3-1a真子代理引擎/M3-2运行器+@触发/M3-3卡片+中间视图/M3-2c编辑UI（全真机）。
- 每 stage：workflow实现 + 3lens对抗审查 + 修复 + 编译 + 真机验证 + commit&push；GitHub 同步、Task_4 留痕。

### ⏳ 收尾待办（非新功能）
1. ✅ **worktree 中文路径 bug 已修复**(子代理 a8181de7 electron 实测,commit)：**真根因(二分定位,推翻 spawn 编码假设)= decodeOutput 误用 GBK 解码 git 输出**——worktree.ts 照搬 command.ts 的 Windows GBK 解码,但 git for windows 路径一律 UTF-8 输出(command 跑 cmd.exe 才 GBK)→git UTF-8 输出被 GBK 误解乱码→ensureGitRepo 乱码 repo root→git -C 乱码路径 fatal。修(worktree.ts 唯一文件):decodeOutput 统一 UTF-8 + runGit 注入 `-c core.quotepath=false`。验证全过(中文 repoRoot list/create/status/remove + worktree 目标路径含中文 + 英文路径不破坏 + 编译 + 清理无残留)。**教训:我之前诊断 spawn cwd/args 编码错了,修复 agent 二分实测纠正——再次印证不盲信表层(含我自己),实测找根因。** [[工作教训：不轻易归因外部/上游，下结论前用真实数据真机验证]]
2. 🟡 **重试×带图边界**(用户统一测)：handleRetry skipUserMessage 复用 store 消息,重试带图是否保留(逻辑应没事,没专测)。
3. 🟡 **整体回归**：M1+M2+M3 全链路真机过一遍(各功能交叉无回归)。
4. 📌 **low 小本本(打磨)**：status工具 WorkflowCard/WorkflowView 重复可抽共享;工作流暂不吃图(GC止血);SettingsPanel 窄侧栏模式名竖排(white-space可改);localStorage 双key冗余(synapse_multi_ai+synapse:multi-ai 待确认清);M3-2 匹配失败走对话消息非toast。
5. 📋 **Stage A 早期项复核**：终端中文✅/模型选择器✅/LibreOffice✅/默认API✅ 已做;窗口IPC状态同步 待确认。
  - M3 遗留小本本：M3-2 工作流暂不吃图(GC止血);status工具 WorkflowCard/View 重复可抽共享;M3-2c 后整体回归 + worktree中文 electron spawn bug(M2-5)专项修。
